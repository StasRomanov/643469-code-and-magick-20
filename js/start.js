...
window.renderStatistics = function (ctx, name, time) {
  ...
  for (var i = 0; i < name.length; i++) {
    if (name[i] === 'Вы') {
      rectRender(ctx, HISTOGRAM_X + (GAP * i), HISTOGRAM_Y, 40, -(time[i] / ratio), 'rgba(255, 0, 0, 1)');
    } else {
      rectRender(ctx, HISTOGRAM_X + (GAP * i), HISTOGRAM_Y, 40, -(time[i] / ratio), 'hsl(242,' + Math.random() * Math.floor(100) + '%' + ' , 47%)');
    }
